...
func Scaling(x, y, z float) Matrix {
	m := NewMatrix(4, 4)
	m.Set(0, 0, x)
	m.Set(1, 1, y)
	m.Set(2, 2, z)
...
func RotationX(rad float) Matrix {
	m := NewMatrix(4, 4)
	m.Set(0, 0, 1)
...
func RotationY(rad float) Matrix {
	m := NewMatrix(4, 4)
...
func RotationZ(rad float) Matrix {
	m := NewMatrix(4, 4)
	m.Set(0, 0, math.Cos(rad))
...
func Shearing(xy, xz, yx, yz, zx, zy float) Matrix {
	m := NewMatrix(4, 4)
	m.Set(0, 0, 1)
	m.Set(1, 1, 1)
	m.Set(2, 2, 1)
